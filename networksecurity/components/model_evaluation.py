...
from networksecurity.utils.main_utils.utils import load_object, save_object, write_yaml_file
...
class ModelEvaluation:
    ...
    def initiate_model_evaluation(self)-> ModelEvaluationArtifact:
        try:
            ...
            train_df = ModelEvaluation.read_data(file_path=valid_train_path)
            test_df = ModelEvaluation.read_data(file_path=valid_test_path)
        
            df = pd.concat([train_df, test_df])
            y_true = df[training_pipeline.TARGET_COLUMN]
            y_true = y_true.replace(-1, 0)
            df = df.drop(training_pipeline.TARGET_COLUMN, axis=1)
